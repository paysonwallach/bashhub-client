...
def json_auth_headers():
    return dict(
        {"Authorization": "Bearer {0}".format(BH_AUTH())}, **json_headers
    )


def base_auth_headers():
    return dict(
        {"Authorization": "Bearer {0}".format(BH_AUTH())}, **base_headers
    )
...
def register_user(register_user):
    ...
    try:
        response = requests.post(
            url, data=register_user.to_JSON(), headers=json_headers
        )
...
def login_user(login_form):
    ...
    try:
        response = requests.post(
            url, data=login_form.to_JSON(), headers=json_headers
        )
...
def register_system(register_system):
    ...
    try:
        response = requests.post(
            url, data=register_system.to_JSON(), headers=json_auth_headers()
        )
        response.raise_for_status()
...
def get_system_information(mac):
    ...
    try:
        response = requests.get(
            url, params=payload, headers=json_auth_headers()
        )
        response.raise_for_status()
...
def patch_system(system_patch, mac):
    ...
    try:
        r = requests.patch(
            url, data=system_patch.to_JSON(), headers=json_auth_headers()
        )
        r.raise_for_status()
...
def search(limit=None, path=None, query=None, system_name=None, unique=None):
    ...
    try:
        ...
        print (
            "Sorry, looks like there's a connection error. Please try again later"
        )
    except Exception as error:
        if r.status_code in (403, 401):
            print ("Permissons Issue. Run bashhub setup to re-login.")
...
def save_command(command):
    ...
    try:
        r = requests.post(
            url, data=command.to_JSON(), headers=json_auth_headers()
        )
    except ConnectionError as error:
